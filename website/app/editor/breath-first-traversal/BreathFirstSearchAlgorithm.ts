    ...
    if (this.nextVertices.length != 0) {
      ...
      for (let i = 0; i < length; i++) {
        ...
        this.commandTracker.push(
          new UpdateVertexRenderingConfiguration(currentVertex, {
            borderColor: levelColor,
            innerColor: "#1c1c1c",
          })
        );

        const allEdges = this.graph.getNeighborEdges(currentVertex);
        ...
        for (const edge of allEdges) {
          ...
          if (currentBatch.includes(otherEnd.id)) {
            this.commandTracker.push(
              new UpdateEdgeRenderingConfiguration(edge, {
      ...
      this.commandTracker.push(new PassiveCommand({ delay: 1 }));
    }
  }
}
=======
