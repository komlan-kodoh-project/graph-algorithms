...
export class EdgeEntity<V, E> extends Container {
  ...
  constructor(
    ...
    this.configuration = new ConfigurationManager({
      ...edgeDefaultDisplayConfiguration,
      ...displayConfiguraton,
    });
  ...
  public updateDisplayConfiguration(
    displayConfiguration: Partial<EdgeDisplayConfiguration<V, E>>
  ): () => void {
  ...
  private handlePositionUpdate() {
    const angle = Math.atan2(
      this.destinationCoordinates.y - this.sourceCoordinates.y,
      this.destinationCoordinates.x - this.sourceCoordinates.x
    );

    const distance = Math.sqrt(
      Math.pow(this.destinationCoordinates.x - this.sourceCoordinates.x, 2) +
        Math.pow(this.destinationCoordinates.y - this.sourceCoordinates.y, 2)
    );
  ...
  private redraw_label() {
    ...
    const text = new Text(configuration.edgeMiddleLabel(this.graphEdge), {
      fontSize: 15,
      align: "center",
      fontFamily: "Arial",
